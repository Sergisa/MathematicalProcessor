...
public class Mathematics {
    ...
    public static double exp(double x){
        return sigma(i -> power(x, i) / factorial(i));
    }

    public static double sin(double x){
        return sigma(n -> (power(-1, n) * power(x, 2 * n + 1)) / (factorial(2 * n + 1)));
    }

    public static double cos(double x){
        return sigma(n -> (power(-1, n) * power(x, 2 * n)) / (factorial(2 * n)));
    }
    ...
    private static double sigma(IterableExpression expr, int iterationStart, int iterations){
        double sum = 0;
    ...
    private static double sigma(IterableExpression expr, int iterationStart){
        return sigma(expr, iterationStart, ITERATIONS);
    }
